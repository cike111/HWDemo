...
#import "HWImageController.h"
#import "HWView.h"
@interface HWImageController ()
...
@implementation HWImageController


- (void)viewDidLoad {
    [super viewDidLoad];

// HWView * view1  =  [[HWView alloc]initWithFrame:CGRectMake(0, 64, 375, 678) withType:1];
...
//    [self.view addSubview:view1];
    ...
    self.Temp_Image = [UIImage imageNamed:@"u=3478543434,1600279262&fm=23&gp=0.jpg"];
...
-(void)loadNav{
    ...
    UILabel * Nav_Label = [[UILabel alloc]initWithFrame:CGRectMake(self.view.bounds.size.width/2-100, 20, 200, 64)];
...
-(void)makeUI{
    ...
    ImageV = [[UIImageView alloc]initWithFrame:CGRectMake(0, 65, self.view.bounds.size.width, self.view.bounds.size.height-65-20)];
...
-(void)tuya{
    ...
    for (int i = 0 ; i<_AllPointArray.count; i++) {
        ...
        for (int j =0 ; j<(int)([_AllPointArray[i] count]-1); j++) {
            ...
            [ImageV.image drawInRect:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-65-20)];
